...
resource "azurerm_virtual_network" "vnet" {
  name                = "${var.corp}-shared-vnet-${var.staging}"
...
resource "azurerm_subnet" "subnet" {
  ...
  security_group       = local.nsg_id
}

# Create public IP
resource "azurerm_public_ip" "publicip" {
  name                = "${var.corp}-${var.component}-extip-${var.staging}"
  location            = var.resource_location
...
resource "azurerm_network_interface" "nic" {
  name                = "${var.corp}-${var.component}-nic-${var.staging}"
  location            = var.resource_location
  resource_group_name = azurerm_resource_group.rg_opencti.name
  tags                = var.tags 

  ip_configuration {
    name                          = "${var.corp}-${var.component}-ipconf-${var.staging}"
    subnet_id                     = "/subscriptions/44af389b-f483-4cfc-9a53-ba8fe50b4ef2/resourceGroups/cybersec-shared-rg-test/providers/Microsoft.Network/virtualNetworks/cybersec-shared-vnet-test/subnets/default" #azurerm_subnet.subnet.id
...
resource "azurerm_network_interface" "elastic_nic" {
  ...
  name                = "${var.corp}-${var.component}-${each.key}-nic-${var.staging}"
  location            = var.resource_location
  resource_group_name = azurerm_resource_group.rg_elastic.name
  tags                = var.tags 

  ip_configuration {
    name                          = "${var.corp}-${var.component}-${each.key}-ipconf-${var.staging}"
    subnet_id                     = "/subscriptions/44af389b-f483-4cfc-9a53-ba8fe50b4ef2/resourceGroups/cybersec-shared-rg-test/providers/Microsoft.Network/virtualNetworks/cybersec-shared-vnet-test/subnets/default" #azurerm_subnet.subnet.id
    private_ip_address_allocation = "Static"
...
resource "azurerm_network_security_group" "nsg" {
  ...
  name                = "${var.corp}-${var.component}-nsg-${var.staging}"
  location            = var.resource_location
  resource_group_name = azurerm_resource_group.rg_opencti.name
  tags                = var.tags 
}
...
resource "azurerm_linux_virtual_machine" "opencti_vm" {
  name                  = "${var.corp}-${var.component}-vm-${var.staging}"
  location              = var.resource_location
  resource_group_name   = azurerm_resource_group.rg_opencti.name
  ...
  os_disk {
    name                 = "${var.corp}-${var.component}-os-disk-${var.staging}"
    caching              = "ReadWrite"
  ...
  computer_name  = "${var.corp}-${var.component}-${var.staging}"
...
resource "azurerm_linux_virtual_machine" "elastic_vm" {
  for_each              = local.elastic_cluster
  name                  = "${var.corp}-${var.component}-${each.key}-vm-${var.staging}"
  location              = var.resource_location
  resource_group_name   = azurerm_resource_group.rg_elastic.name
  ...
  os_disk {
    name                 = "${var.corp}-${var.component}-${each.key}-os-disk-${var.staging}"
    caching              = "ReadWrite"
...
resource "azurerm_managed_disk" "opencti_data" {
  name                 = "${var.corp}-${var.component}-data-disk-${var.staging}"
  location             = var.resource_location
  resource_group_name  = azurerm_resource_group.rg_opencti.name
...
resource "azurerm_managed_disk" "elastic_data" {
  for_each             = local.elastic_cluster
  name                 = "${var.corp}-${var.component}-${each.key}-data-disk-${var.staging}"
